...
export default function Sidebar() {
  ...
  return (
    <aside
      className={`h-[1280px] bg-white border-r-3 border-gray-300 py-4 px-3 flex flex-col justify-between transition-all duration-300 ease-in-out ${expanded ? "w-64" : "w-20"}`}
    >
      <SidebarContext.Provider value={{ expanded }}>
        <div>
          ...
          <div className="flex items-center justify-between pb-1 mb-4 border-3 rounded-lg border-gray-300">
            ...
            <button onClick={() => setExpanded((curr) => !curr)} className="p-1.5 hover:bg-gray-100 rounded-lg">
          ...
          <div className="space-y-2 rounded-lg p-2 border-3 border-gray-300">
            <div
              className={`flex items-center justify-between px-2 overflow-hidden transition-all ${expanded ? "w-full" : "w-0 h-0"}`}
            >
          ...
          {/* "Thoughts" Block - Now positioned below Projects */}
          <div
            className={`bg-gray-200 p-4 rounded-xl text-center relative transition-all mt-[60px] ${expanded ? "" : "h-0 p-0"}`}
          >
            <div
              className={`w-12 h-12 bg-gray-200 rounded-full mx-auto flex items-center justify-center absolute -top-6 left-1/2 -translate-x-1/2 transition-all ${!expanded && "opacity-0"}`}
            >
